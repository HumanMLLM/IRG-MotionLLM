...
class Text2MotionDataset(data.Dataset):
    def __init__(self, dataset_name, type, w_vectorizer, feat_bias = 5, max_text_len = 20, unit_length = 4, is_debug=False, return_all_captions=False, return_neg_motion=False):
        ...
        self.return_neg_motion = return_neg_motion
        if dataset_name == 't2m':
    ...
    def save_data_dict(self, path):
        ...
        json.dump(saved_dict, open(path, 'w'), indent=4)  # self.save_data_dict('/mnt/data1/yuanming/Code/Motion_Gen/Motion-Agent/dataset/train_data_dict.json')

    def __getitem__(self, item):
        if self.return_neg_motion:
            neg_name = random.choice(self.name_list)
            neg_motion, neg_m_length = self.data_dict[neg_name]['motion'], self.data_dict[neg_name]['length']
            ...
            if coin2 == 'double':
                neg_m_length = (neg_m_length // self.unit_length - 1) * self.unit_length
            elif coin2 == 'single':
                neg_m_length = (neg_m_length // self.unit_length) * self.unit_length
            idx = random.randint(0, len(neg_motion) - neg_m_length)
            neg_motion = neg_motion[idx:idx+neg_m_length]

            "Z Normalization"
            neg_motion = (neg_motion - self.mean) / self.std

            if neg_m_length < self.max_motion_length:
                neg_motion = np.concatenate([neg_motion,
                                        np.zeros((self.max_motion_length - neg_m_length, neg_motion.shape[1]))
                                        ], axis=0)
        ...
        if self.return_neg_motion:
            if self.return_all_captions:
                return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens), name, all_captions, neg_motion, neg_m_length
            else:
                return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens), name, neg_motion, neg_m_length
