...
from models.motion_vqvae.encdec import Encoder, Decoder
from models.motion_vqvae.quantize_cnn import QuantizeEMAReset, Quantizer, QuantizeEMA, QuantizeReset
...
class VQVAE_251(nn.Module):
    ...
    def preprocess(self, x):
        # (bs, T, Jx3) -> (bs, Jx3, T)
        # x = x.permute(0,2,1).float()      # 这个是原本的实现，但是如果在训练的时候将模型转为bfloat16 就会出问题，所以这里改为下面的实现
        valid_dtype = next(self.encoder.parameters()).dtype
        x = x.permute(0,2,1).to(dtype=valid_dtype)
        return x
